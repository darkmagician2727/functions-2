...
const expressionfindGrape = function(arr){
    for(let i=0; i<arr.length; i++) {
        if(arr[i].color === "purple") {
            return console.log(`The fruit with index ${arr.indexOf(arr[i])} is a grape`)
        } 
...
const parameterfunction = (param1, param2) => {console.log(`The first parameter is ${param1}. The second parameter is ${param2} `)}


// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.

parameterfunction(1, "string")


////////// PROBLEM 3 //////////
/*
Write a function called 'greeting' that takes 3 parameters: firstName, lastName, and a callback function. In the function, 
invoke the callback, passing in a concatenation of firstName and lastName. Then, outside of the greeting function, 
invoke the greeting function, passing in the value firstName and lastName of your choice, 
...
// callback2(greeting(tj, albrecht))


function greeting(firstName, lastName, callb){
    let fullName = firstName + ` ` + lastName
    callb(fullName)
}

function callback1(fullName){
    console.log(`Hello, my full name is ${fullName}`)
}

// function callback2(fullName){
//     console.log(`No, my full name is ${fullName}`)
// }

// INVOKE 'GREETING FUNCTION' HERE

greeting("Dwayne", "The rock", callback1)
...
calculate a new total cost (original price plus 20% tax), then push it to the 'totalCost' array. After the loop, 
invoke the callback function, passing in the 'totalCost' array.
*/
...
and a callback function (passing in 'totalCost' as a param) that will print "The new array plus tax = [totalCost]"
*/

// CODE HERE

function NewPricePrint(arr){
    console.log(`The new array plus tax = ${arr}`)
}

pricesPlusTax(prices, NewPricePrint)
